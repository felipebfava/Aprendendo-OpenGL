...
def desenhaCirculo2d(raio, segmentos):
    ...
    glTranslatef(transla_x_circ, transla_y_circ, 0)
...
def desenharQuadrado2d():
    ...
    # Para aplicar movimento na estrutura
    # Precisa ser antes de definir a estrutura em glBegin()
    glTranslatef(transla_x_quad, transla_y_quad, 0)
    ...
    # Alterando a posição em x,y,z
    # Como faremos um quadrado 2d usaremos glVertex2f()
...
def colisaoCircxQuad():
    # Para verificar a colisão entre um quadrado x círculo
    # precisamos saber as distâncias entre os centros deles
    # Usando:
    # distancia_em_x = x_circulo - x_quadrado
    # distancia_em_y = y_circulo - y_quadrado

    raio_circulo = 0.1
    metade_lado_quadrado = 0.25

    # Pega o valor absoluto / distância até zero de um número
    dx = abs(transla_x_circ - transla_x_quad)
    dy = abs(transla_y_circ - transla_y_quad)

    # isso dará o mesmo valor próximo de 0.35
    # if dx < (metade_lado_quadrado + raio_circulo) and dy < (metade_lado_quadrado + raio_circulo):
    #     print("COLISÃO")

    # impõe um limite de proximidade
    if dx < 0.33 and dy < 0.33:
        print("COLISÃO")
...
def render():
    ...
    # Chama a função de desenhar o quadrado 2d
    desenharQuadrado2d()

    # Chama a função de colisão das estruturas
    # colisaoCircxQuad()
...
def teclado(window, key, scancode, action, mods):
    ...
    global transla_x_circ, transla_y_circ
    global transla_x_quad, transla_y_quad

    # se a ação for de pressionar (tecla pressionada) faça
    if action == glfw.PRESS:

        # Teclas para movimentar o Círculo
        ...
        if key == glfw.KEY_W: # se esta tecla for W faça
            transla_y_circ += 0.01 # sobe
        
        if key == glfw.KEY_S:
            transla_y_circ -= 0.01 # desce
        
        if key == glfw.KEY_A:
            transla_x_circ -= 0.01 # esquerda
        
        if key == glfw.KEY_D:
            transla_x_circ += 0.01 # direita
        
        # Teclas para movimentar o Quadrado
        # UP sobe, DOWN desce, LEFT move para esquerda,RIGHT move para direita
        # para subir(+) ou descer(-) precisa alterar o valor de y
        # para a esquerda x diminui(-) e para a direita x aumenta (+)
        
        if key == glfw.KEY_UP: # se esta tecla for W faça
            transla_y_quad += 0.01 # sobe
        
        if key == glfw.KEY_DOWN:
            transla_y_quad -= 0.01 # desce
        
        if key == glfw.KEY_LEFT:
            transla_x_quad -= 0.01 # esquerda
        
        if key == glfw.KEY_RIGHT:
            transla_x_quad += 0.01 # direita
